...
(function () {
  ...
  function _renderAll(data) {
    ...
    el('brand-speed-sub').textContent    = speed.hasData    ? `${speed.onTime} on-time από ${speed.total}` : 'Απαιτείται πεδίο: Actual Delivery Date';
    el('brand-quality-sub').textContent  = quality.hasData  ? `${quality.sent} με proof από ${quality.total}` : 'Απαιτείται πεδίο: Temp Graph Sent';
    el('brand-anxiety-sub').textContent  = `Στόχος: 0 inbound calls / εβδομάδα`;
    el('brand-speed-val').style.color    = speed.hasData    ? _colorScore(speed.value, 98)       : '#7A92B0';
    el('brand-quality-val').style.color  = quality.hasData  ? _colorScore(quality.value, 100)    : '#7A92B0';
  ...
  window._ceoSaveRevTarget = function() {
    const v = parseFloat(document.getElementById('rev-target-input').value) || 0;
  ...
  function _donut(id, dataVals, colors) {
    ...
    _charts[id] = new Chart(el, {
      ...
      options: {
        plugins: { legend: { display: true, position: 'bottom', labels: { color: '#7A92B0', font: { size: 10 }, padding: 8 } }, tooltip: { callbacks: { label: ctx => ` ${ctx.label}: ${ctx.parsed}` } } },
        cutout: '60%',
  ...
  function _sparkline(id, weeklyData, color, targetVal, lowerIsBetter) {
    ...
    _charts[id] = new Chart(el, {
      ...
      options: {
        ...
        scales: {
          x: { ticks: { color: '#7A92B0', font: { size: 9 } }, grid: { color: '#0d1e2e' } },
          y: { ticks: { color: '#7A92B0', font: { size: 9 }, callback: v => v + '%' }, grid: { color: '#1c2a3a' }, min: 0, max: 100 }
        },
        ...
        datasets: [
          {
            ...
            backgroundColor: 'rgba(2,132,199,0.08)', fill: true,
  ...
  function _shellHTML() {
    ...
    return `
<style>
.ceo-wrap { padding: 20px 24px 40px; min-height: 100vh; }
.ceo-header { display:flex; align-items:center; gap:16px; margin-bottom:24px; flex-wrap:wrap; }
.ceo-title-block { flex:1; }
.ceo-title { font-family:'Syne',sans-serif; font-size:22px; font-weight:700; color:#e2e8f0; letter-spacing:0.5px; }
.ceo-subtitle { font-size:11px; color:#7A92B0; letter-spacing:1.5px; text-transform:uppercase; margin-top:2px; }
.ceo-period-sel { display:flex; gap:4px; background:#0d1e2e; padding:4px; border-radius:8px; }
.ceo-period-btn { padding:6px 14px; font-size:12px; font-family:'DM Sans',sans-serif; border:none; border-radius:6px; cursor:pointer; background:transparent; color:#7A92B0; transition:all 0.2s; }
.ceo-period-btn.active { background:#0284C7; color:#fff; font-weight:500; }
.ceo-period-btn:hover:not(.active) { color:#e2e8f0; background:#1c2a3a; }
#ceo-updated { font-size:11px; color:#7A92B0; white-space:nowrap; }

/* Brand Promises */
.ceo-brand-row { display:grid; grid-template-columns:repeat(3,1fr); gap:16px; margin-bottom:20px; }
.ceo-brand-card { background:#0d1e2e; border-radius:12px; padding:24px; border:1px solid #1c2a3a; position:relative; overflow:hidden; }
.ceo-brand-card::before { content:''; position:absolute; top:0; left:0; right:0; height:3px; border-radius:12px 12px 0 0; }
...
.ceo-brand-label { font-size:10px; font-weight:600; letter-spacing:1.8px; text-transform:uppercase; color:#7A92B0; margin-bottom:4px; }
.ceo-brand-name  { font-family:'Syne',sans-serif; font-size:14px; font-weight:700; color:#e2e8f0; margin-bottom:16px; }
.ceo-gauge-wrap  { position:relative; height:80px; display:flex; flex-direction:column; align-items:center; justify-content:flex-end; }
.ceo-gauge-wrap canvas { max-height:80px; }
.ceo-brand-big   { font-family:'Syne',monospace; font-size:42px; font-weight:800; line-height:1; margin-bottom:4px; }
.ceo-brand-sub   { font-size:11px; color:#7A92B0; min-height:30px; }
.ceo-target      { font-size:10px; color:#334155; margin-top:6px; }

/* Anxiety manual entry */
.anxiety-entry { display:flex; align-items:center; gap:8px; margin-top:12px; padding-top:12px; border-top:1px solid #1c2a3a; }
.anxiety-entry input { width:60px; padding:4px 8px; background:#0a1628; border:1px solid #334155; border-radius:6px; color:#e2e8f0; font-size:13px; text-align:center; }
.anxiety-entry button { padding:4px 10px; background:#0284C7; color:#fff; border:none; border-radius:6px; cursor:pointer; font-size:11px; }

/* Quadrant grid */
.ceo-quadrants { display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:20px; }
.ceo-quad { background:#0d1e2e; border-radius:12px; padding:20px; border:1px solid #1c2a3a; }
.ceo-quad-title { font-family:'Syne',sans-serif; font-size:11px; font-weight:700; letter-spacing:2px; text-transform:uppercase; color:#7A92B0; margin-bottom:16px; display:flex; align-items:center; gap:8px; }
.ceo-quad-title span.dot { width:6px; height:6px; border-radius:50%; display:inline-block; }
.quad-people   .dot { background:#0284C7; }
.quad-strategy .dot { background:#22c55e; }
.quad-exec     .dot { background:#f59e0b; }
.quad-cash     .dot { background:#7c3aed; }

/* KPI rows inside quads */
.ceo-kpi-row { display:flex; align-items:flex-start; gap:12px; padding:10px 0; border-bottom:1px solid #0a1628; }
.ceo-kpi-row:last-child { border-bottom:none; }
.ceo-kpi-num { font-family:'Syne',monospace; font-size:26px; font-weight:800; line-height:1; white-space:nowrap; min-width:90px; }
.ceo-kpi-info { flex:1; }
.ceo-kpi-label { font-size:11px; font-weight:600; letter-spacing:1px; text-transform:uppercase; color:#94a3b8; }
.ceo-kpi-sub   { font-size:11px; color:#7A92B0; margin-top:2px; }

/* Progress bars */
.ceo-bar-track { height:4px; background:#1c2a3a; border-radius:2px; margin-top:6px; }
.ceo-bar-fill  { height:4px; border-radius:2px; transition:width 0.6s ease; }

/* Charts */
.ceo-chart-wrap { margin-top:8px; height:80px; }
.ceo-chart-wrap canvas { max-height:80px; }

/* Tables */
.ceo-mini-table { width:100%; border-collapse:collapse; font-size:12px; }
.ceo-mini-table td { padding:6px 4px; border-bottom:1px solid #0a1628; }
.ceo-mini-table tr:last-child td { border-bottom:none; }

/* Revenue target setter */
.rev-target-row { display:flex; align-items:center; gap:8px; margin-top:8px; padding:8px; background:#0a1628; border-radius:6px; }
.rev-target-row label { font-size:10px; color:#7A92B0; white-space:nowrap; }
.rev-target-row input { flex:1; padding:4px 8px; background:#0d1e2e; border:1px solid #334155; border-radius:4px; color:#e2e8f0; font-size:12px; }
.rev-target-row button { padding:4px 10px; background:#334155; color:#e2e8f0; border:none; border-radius:4px; cursor:pointer; font-size:11px; white-space:nowrap; }
.rev-target-row button:hover { background:#0284C7; }

/* Nakis */
.ceo-nakis { background:linear-gradient(135deg,#0d1e2e 0%,#0a1f30 100%); border-radius:12px; padding:20px; border:1px solid #1c2a3a; margin-bottom:20px; }
.ceo-nakis-title { font-size:10px; font-weight:700; letter-spacing:2px; text-transform:uppercase; color:#38BDF8; margin-bottom:12px; display:flex; align-items:center; gap:8px; }
.nakis-line { font-size:13px; color:#94a3b8; line-height:1.6; padding:4px 0; border-bottom:1px solid #0a1628; }
.nakis-line:last-child { border-bottom:none; }
.nakis-line strong { color:#e2e8f0; }

/* Section dividers */
.ceo-section-label { font-size:10px; font-weight:700; letter-spacing:2px; text-transform:uppercase; color:#334155; margin:0 0 12px; }

@media (max-width:768px) {
  .ceo-brand-row { grid-template-columns:1fr; }
  .ceo-quadrants  { grid-template-columns:1fr; }
  .ceo-brand-big { font-size:32px; }
  .ceo-kpi-num   { font-size:20px; min-width:70px; }
  .ceo-header { flex-direction:column; align-items:flex-start; }
  .ceo-period-sel { width:100%; justify-content:space-between; }
  .ceo-wrap { padding: 12px 12px 40px; }
}
@media (max-width:480px) {
  .ceo-period-btn { padding:5px 8px; font-size:10px; }
}
</style>
...
<div class="ceo-wrap">
  ...
  <div class="ceo-brand-row">
    ...
    <div class="ceo-brand-card speed">
      ...
      <div class="ceo-brand-sub" id="brand-speed-sub">Φόρτωση...</div>
    ...
    <div class="ceo-brand-card quality">
      ...
      <div class="ceo-brand-sub" id="brand-quality-sub">Φόρτωση...</div>
    ...
    <div class="ceo-brand-card anxiety">
      ...
      <div class="ceo-brand-sub" id="brand-anxiety-sub">Φόρτωση...</div>
      <div class="ceo-target">Στόχος: 0 inbound calls / εβδομάδα</div>
      <div class="anxiety-entry">
        <span style="font-size:11px;color:#7A92B0">Αυτή την εβδομάδα:</span>
        <input type="number" id="anxiety-input" min="0" max="99" placeholder="0">
  ...
  <!-- Quadrants 2x2 -->
  <div class="ceo-section-label" style="margin-top:24px">Scaling Up Framework</div>
  <div class="ceo-quadrants">
    ...
    <div class="ceo-quad quad-people">
      ...
      <div class="ceo-kpi-row">
        ...
        <div style="flex:1">
          <div class="ceo-kpi-num" id="people-avail-val" style="font-size:20px">—</div>
          <div class="ceo-kpi-label" style="font-size:10px">Διαθέσιμοι</div>
          <div class="ceo-kpi-sub" id="people-avail-sub" style="font-size:10px"></div>
        </div>
      </div>
      ...
      <div class="ceo-kpi-row">
        ...
        <div style="flex:1; max-width:120px; height:80px;"><canvas id="chart-partner"></canvas></div>
      </div>

      <div style="margin-top:8px;">
        <div class="ceo-kpi-label" style="margin-bottom:6px">Workload Distribution</div>
    ...
    <div class="ceo-quad quad-strategy">
      ...
      <div class="ceo-kpi-row">
        <div style="flex:1">
          ...
          <div class="rev-target-row">
            <label>Μηνιαίος Στόχος €</label>
            <input type="number" id="rev-target-input" placeholder="π.χ. 150000">
            <button onclick="_ceoSaveRevTarget()">Ορισμός</button>
          </div>
        </div>
      </div>
      ...
      <div style="margin-top:8px;">
        <div class="ceo-kpi-label" style="margin-bottom:6px">Top 5 Πελάτες (Revenue)</div>
        <table class="ceo-mini-table">
          <tbody id="strat-clients-body"><tr><td colspan="4" style="color:#7A92B0;text-align:center">Φόρτωση...</td></tr></tbody>
        </table>
      </div>
    </div>
    ...
    <div class="ceo-quad quad-exec">
      ...
      <div class="ceo-kpi-row">
        <div style="flex:1">
          ...
          <div style="height:100px; margin-top:8px;"><canvas id="chart-ontime"></canvas></div>
        </div>
      </div>
      ...
      <div class="ceo-kpi-row">
        ...
        <div style="margin-left:auto">
          <div class="ceo-kpi-num" id="exec-assign-val" style="font-size:20px;text-align:right">—</div>
          <div class="ceo-kpi-label" style="font-size:10px;text-align:right">Assigned</div>
          <div class="ceo-kpi-sub" id="exec-assign-sub" style="font-size:10px;text-align:right"></div>
        </div>
      </div>
    </div>
    ...
    <div class="ceo-quad quad-cash">
      ...
      <div class="ceo-kpi-row">
        <div>
          <div class="ceo-kpi-num" id="cash-revenue-val" style="font-size:28px">—</div>
      ...
      <div class="ceo-kpi-row">
        ...
        <div style="margin-left:auto">
          ...
          <div class="ceo-kpi-label" style="font-size:10px;text-align:right">Maintenance</div>
          <div class="ceo-kpi-sub" id="cash-maint-sub" style="font-size:10px;text-align:right"></div>
        </div>
      </div>
      ...
      <div style="margin-top:8px;">
        <div class="ceo-kpi-label" style="margin-bottom:6px">Top 3 Ζημιογόνες Διαδρομές</div>
        <table class="ceo-mini-table">
          <thead><tr>
            <th style="color:#7A92B0;font-size:9px;text-align:left;padding:4px">Διαδρομή</th>
            <th style="color:#7A92B0;font-size:9px;text-align:left;padding:4px">Έσοδο</th>
            <th style="color:#7A92B0;font-size:9px;text-align:left;padding:4px">Κόστος</th>
            <th style="color:#7A92B0;font-size:9px;text-align:left;padding:4px">Ζημία</th>
          </tr></thead>
          <tbody id="cash-loss-body"><tr><td colspan="4" style="color:#7A92B0;text-align:center">Φόρτωση...</td></tr></tbody>
        </table>
      </div>
    </div>
  ...
  <div class="ceo-nakis">
    ...
    <div id="nakis-commentary" style="color:#7A92B0">Φόρτωση σχολίων...</div>
  </div>
