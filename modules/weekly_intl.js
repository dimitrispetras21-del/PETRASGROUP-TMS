// ═══════════════════════════════════════════════════════════════════════
// WEEKLY INTERNATIONAL — v4
//
// LAYOUT (3 columns, assignment always visible):
//
//  ┌─ Export (flex) ──────────────┬─ Assignment (220px) ─┬─ Import (flex) ─┐
//  │ CLIENT → DESTINATION         │ □ Partner            │ drag here /chip │
//  │ 📅 load→del  📦 N pal  🌡 N°C │ [Truck ▼]           │                 │
//  │                              │ [Trailer ▼]          │                 │
//  │ GROUPAGE ×N                  │ [Driver ▼]           │                 │
//  │  · sub line                  │ [🔗 Create Trip]     │                 │
//  └──────────────────────────────┴──────────────────────┴─────────────────┘
// ═══════════════════════════════════════════════════════════════════════
...
const WINTL = {
  ...
  rows:      [],
  shelf:     [],
  ui:        { shelfFilter:'', shelfCollapsed:false },
  _seq:      0,
  _assetsOk: false,
};
...
(function injectStyles(){
  if(document.getElementById('wi4-styles')) return;
  ...
  s.textContent=`
    ...
    .wi-row-grid:hover { background: rgba(0,0,0,0.008); }
    .wi-row-grid.is-saved   { background: rgba(5,150,105,0.03); }
    .wi-row-grid.is-dirty   { background: rgba(59,130,246,0.03); }
    ...
    .wi-c-imp.drop-hover {
      ...
      outline: 1.5px dashed rgba(217,119,6,0.35);
      outline-offset: -3px;
    }

    /* ── export text ── */
    .wi-route   { font-size: 12px; font-weight: 600; color: var(--text); line-height: 1.3; }
    .wi-route b { color: var(--text); }
    .wi-route .arr { color: var(--text-dim); margin: 0 4px; }
    .wi-meta    { display: flex; flex-wrap: wrap; gap: 6px; margin-top: 1px; }
    .wi-meta-i  { font-size: 10.5px; color: var(--text-dim); }
    .wi-meta-i span { margin-right: 1px; }

    /* ── groupage sub-lines ── */
    .wi-sub { display: flex; flex-direction: column; gap: 2px; margin-top: 4px; padding-top: 5px;
              border-top: 1px dashed var(--border); }
    .wi-sub-line { font-size: 10.5px; color: var(--text-mid); padding-left: 6px;
                   position: relative; }
    .wi-sub-line::before { content:'·'; position:absolute; left:0; color:var(--text-dim); }

    /* ── badges ── */
    .wi-b { display:inline-block; font-size:8.5px; font-weight:700; letter-spacing:1px;
            text-transform:uppercase; padding:1px 5px; border-radius:3px; }
    .wi-b-vx { background:rgba(99,102,241,0.1);color:rgba(99,102,241,0.85);border:1px solid rgba(99,102,241,0.18); }
    .wi-b-gr { background:rgba(14,165,233,0.1);color:rgba(14,165,233,0.85);border:1px solid rgba(14,165,233,0.18); }
    .wi-b-ok { background:rgba(5,150,105,0.1);color:rgba(5,150,105,0.85);border:1px solid rgba(5,150,105,0.16); }
    ...
    .wi-partner-toggle input { cursor: pointer; accent-color: var(--text); }

    /* ── searchable dropdown ── */
    .wi-sd { position: relative; }
    .wi-sd-inp {
      width: 100%; padding: 5px 8px; font-size: 11px; border-radius: 5px;
      border: 1px solid var(--border-mid); background: var(--bg-card);
      color: var(--text); outline: none;
    }
    .wi-sd-inp:focus { border-color: rgba(11,25,41,0.3); box-shadow: 0 0 0 2px rgba(11,25,41,0.06); }
    .wi-sd-list {
      display:none; position:fixed; z-index:9999; min-width:200px; max-height:200px;
    ...
    /* ── rates (partner mode) ── */
    .wi-rate-row { display:flex; gap:6px; }
    .wi-rate-inp {
      width:92px; padding:5px 8px; font-size:11px; border-radius:5px;
      border:1px solid var(--border-mid); background:var(--bg-card);
      color:var(--text); outline:none;
    }
    ...
    .wi-btn-create.saving { opacity:0.5; pointer-events:none; }
    .wi-btn-delete {
      width:100%; padding:4px 0; font-size:10px;
      border:1px solid rgba(220,38,38,0.25); border-radius:5px; cursor:pointer;
      background:none; color:var(--danger); transition:background 0.1s;
    }
    .wi-btn-delete:hover { background:var(--danger-bg); }

    /* ── import chip (in row) ── */
    .wi-imp-chip {
      background:rgba(217,119,6,0.07); border:1px solid rgba(217,119,6,0.2);
      border-radius:6px; padding:7px 10px; position:relative; cursor:grab;
    }
    .wi-imp-chip:active { cursor:grabbing; }
    .wi-imp-rm {
      position:absolute; top:4px; right:6px; font-size:11px; cursor:pointer;
      color:var(--text-dim); opacity:0.6; line-height:1;
    }
    .wi-imp-rm:hover { opacity:1; color:var(--danger); }
    .wi-imp-empty { font-size:10.5px; color:var(--border-dark,#cbd5e1); letter-spacing:0.3px; }
    ...
    .wi-shelf-chip:hover { box-shadow:0 3px 10px rgba(0,0,0,0.09); transform:translateY(-1px); }
    ...
    .wi-ctx-i { display:block; width:100%; padding:7px 14px; text-align:left;
    ...
    .wi-ctx-i.danger { color:var(--danger); }
    .wi-ctx-sep  { height:1px; background:var(--border); margin:4px 0; }
    .wi-ctx-head { padding:4px 14px 2px; font-size:9px; font-weight:700;
...
function _wiCurrentWeek(){
  const d=new Date(),y=d.getFullYear(),jan1=new Date(y,0,1);
...
async function renderWeeklyIntl(){
  ...
  document.getElementById('content').innerHTML=`
    <div style="display:flex;align-items:center;justify-content:center;gap:10px;height:160px;color:var(--text-dim);font-size:13px">
...
function _wiPaint(){
  ...
  document.getElementById('content').innerHTML=`
    ...
    <!-- IMPORT SHELF (only when unmatched exist) -->
    ${unmatched ? `
    <div style="margin-bottom:10px;background:var(--bg-card);
      ...
      <div style="display:flex;align-items:center;gap:10px;padding:8px 14px;
        ...
        <span style="background:rgba(217,119,6,0.12);color:var(--warning);
                     font-size:10px;font-weight:700;padding:1px 8px;border-radius:10px">${unmatched}</span>
        ${unmatched>4?`
        <input type="text" placeholder="search…" value="${WINTL.ui.shelfFilter}"
               oninput="WINTL.ui.shelfFilter=this.value;_wiPaintShelf()"
               onclick="event.stopPropagation()"
               style="margin-left:4px;padding:4px 9px;font-size:11px;border-radius:6px;
    ...
    <div style="border:1px solid var(--border-mid);border-radius:10px;overflow:hidden;
      ...
      <!-- header -->
      <div class="wi-tbl-hdr">
        <div class="wi-tbl-hdr-cell" style="color:var(--success)">
          ↑ Export (${expN})
          <span style="font-size:9px;font-weight:400;color:var(--text-dim);
                       text-transform:none;letter-spacing:0;margin-left:8px">sorted by loading · right-click to group</span>
        </div>
        <div class="wi-tbl-hdr-cell" style="color:var(--text-dim);
             border-left:1px solid var(--border-mid);border-right:1px solid var(--border-mid);text-align:center">
          Assignment
        </div>
        <div class="wi-tbl-hdr-cell" style="color:var(--warning)">
          ↓ Import
          <span style="font-size:9px;font-weight:400;color:var(--text-dim);
                       text-transform:none;letter-spacing:0;margin-left:8px">drag from shelf</span>
        </div>
      </div>
...
function _wiShelfHTML(){
  ...
  return `<div style="display:flex;flex-wrap:wrap;gap:8px">${vis.map(_wiShelfChip).join('')}</div>`;
}
function _wiPaintShelf(){
  const el=document.getElementById('wi-shelf');
  if(el) el.innerHTML=_wiShelfHTML();
}
...
function _wiShelfChip(r){
  const f=r.fields;
  const from=_wiClean(f['Loading Summary']||'—').slice(0,26);
  const to  =_wiClean(f['Delivery Summary']||'—').slice(0,22);
  const pals=f['Total Pallets']||0;
  const delDt=_wiFmt(f['Delivery DateTime']);
  return `
    <div class="wi-shelf-chip" draggable="true" data-impid="${r.id}"
      ...
      <div style="font-size:10px;color:var(--text-mid);margin-top:1px">${pals} pal · del ${delDt}</div>
    </div>`;
}

/* ─────────────────────────────────────────────
   ALL ROWS WITH DATE SEPARATORS
───────────────────────────────────────────── */
function _wiAllRowsHTML(){
  ...
  WINTL.rows.forEach((row,i)=>{
    ...
    if(label && label!==lastDate){
      lastDate=label;
      html+=`<div class="wi-date-sep">${label}</div>`;
    }
...
function _wiRowHTML(row,i){
  ...
  const rowCls=row.saved?'is-saved':row.dirty?'is-dirty':'';
  ...
  // Groupage: show sub-lines for each export
  const subLines=isGroup ? `
    <div class="wi-sub">
      ${exps.map(r=>{
        const f=_wiClean(r.fields['Loading Summary']||'—');
        const t=_wiClean(r.fields['Delivery Summary']||'—');
        const p=r.fields['Total Pallets']||0;
        return `<div class="wi-sub-line">${f} → ${t} · ${p} pal</div>`;
      }).join('')}
    </div>` : '';

  const expCell=`
    <div class="wi-c-exp" oncontextmenu="_wiCtx(event,${row.id})">
      <div class="wi-num-dot">
        ...
        ${row.saved && row.tripNo?`<span style="font-size:10px;color:var(--text-dim)">#${row.tripNo}</span>`:''}
      </div>
      <div class="wi-route">
        <b>${fromStr}</b>
        <span class="arr">→</span>
      ...
      <div class="wi-meta">
        <span class="wi-meta-i"><span>📅</span>${loadDt}→${delDt}</span>
        <span class="wi-meta-i"><span>📦</span>${pals} pal</span>
        ${temp?`<span class="wi-meta-i"><span>🌡</span>${temp}°C</span>`:''}
      </div>
      ${subLines}
    </div>`;

  /* ── ASSIGNMENT CELL ── */
  const asgnCell=_wiAsgnCellHTML(row);

  /* ── IMPORT CELL ── */
  const impCell=`
    <div id="wi-imp-${row.id}" class="wi-c-imp"
         ondragover="event.preventDefault();document.getElementById('wi-imp-${row.id}').classList.add('drop-hover')"
         ondragleave="document.getElementById('wi-imp-${row.id}').classList.remove('drop-hover')"
         ondrop="_wiDrop(event,${row.id})">
      ${imp ? `
        <div class="wi-imp-chip" draggable="true" data-impid="${imp.id}"
             ondragstart="_wiDragStart(event,'${imp.id}')">
          <span class="wi-imp-rm" onclick="_wiRemoveImport(${row.id})">✕</span>
          <div style="font-size:11px;font-weight:600;color:var(--text);padding-right:16px">
            ${_wiClean(imp.fields['Loading Summary']||'—')}
          </div>
          <div style="font-size:10px;color:var(--text-dim)">
            → ${_wiClean(imp.fields['Delivery Summary']||'—')}
          </div>
          <div style="font-size:10px;color:var(--text-mid);margin-top:2px">
            ...
            · 📦 ${imp.fields['Total Pallets']||0} pal
          </div>
        </div>` :
        `<span class="wi-imp-empty">— drag import here —</span>`}
    </div>`;

  return `
    <div id="wi-row-${row.id}" class="wi-row-grid ${rowCls}">
      ${expCell}
      ${asgnCell}
      ${impCell}
    </div>`;
...
function _wiAsgnCellHTML(row){
  ...
  if(!isPartner){
    fieldsHTML=`
      ${_wiSdrop('tk',row.id,trucks,  row.truckId,   row.truckPlate||'Truck…')}
      ${_wiSdrop('tl',row.id,trailers,row.trailerId, row.trailerPlate||'Trailer…')}
    ...
    fieldsHTML=`
      ...
      <div class="wi-rate-row">
        <div>
          <div style="font-size:9px;color:var(--text-dim);margin-bottom:2px">EXP €</div>
          <input type="number" class="wi-rate-inp" placeholder="0.00"
                 value="${row.partnerRateExp||''}"
                 oninput="_wiField(${row.id},'partnerRateExp',this.value)"/>
        </div>
        <div>
          <div style="font-size:9px;color:var(--text-dim);margin-bottom:2px">IMP €</div>
          <input type="number" class="wi-rate-inp" placeholder="0.00"
  ...
  const actionBtn = canFull ? (row.saved
    ? `<button class="wi-btn-create" id="wi-btn-${row.id}"
               onclick="_wiSaveTrip(${row.id})">Update Trip</button>
       <button class="wi-btn-delete" onclick="_wiDeleteTrip(${row.id})">Delete</button>`
    : `<button class="wi-btn-create" id="wi-btn-${row.id}"
               onclick="_wiSaveTrip(${row.id})">🔗 Create Trip</button>`)
    : '';

  return `<div class="wi-c-asgn">${partnerToggle}${fieldsHTML}${actionBtn}</div>`;
}
...
function _wiSdrop(prefix,rowId,arr,selectedId,placeholder){
  ...
  const opts=arr.map(x=>{
    ...
    return `<div class="wi-sd-opt ${x.id===selectedId?'selected':''}" data-id="${x.id}" data-lbl="${lbl}">${lbl}</div>`;
  }).join('');
  return `
...
// Event delegation for option picks
...
function _wiSdOpen(id){
  ...
  Object.assign(list.style,{display:'block',left:`${r.left}px`,top:`${r.bottom+2}px`,
...
function _wiSdPick(id,recId,label){
  ...
  const fldMap={tk:'truckId',tl:'trailerId',dr:'driverId',pt:'partnerId'};
  const lblMap={tk:'truckPlate',tl:'trailerPlate',dr:'driverName',pt:'partnerName'};
  if(fldMap[prefix]&&!isNaN(rowId)){
    _wiField(rowId,fldMap[prefix],recId);
    _wiField(rowId,lblMap[prefix],label);
  }
}
...
function _wiField(rowId,field,val){
  ...
  if(row){row[field]=val; if(!row.saved) row.dirty=true;}
}
function _wiSetCarrier(rowId,type){
  const row=WINTL.rows.find(r=>r.id===rowId); if(!row) return;
  row.carrierType=type; if(!row.saved) row.dirty=true;
  _wiRepaintRow(rowId);
}
...
function _wiDrop(e,rowId){
  e.preventDefault();
  document.getElementById('wi-imp-'+rowId)?.classList.remove('drop-hover');
  ...
  row.importId=impId; if(!row.saved) row.dirty=true;
  _wiPaintShelf();
  _wiRepaintRow(rowId);
}
...
function _wiCtx(e,rowId){
  ...
  const item=(lbl,fn,danger=false)=>
    `<button class="wi-ctx-i ${danger?'danger':''}" onclick="${fn};_wiCtxClose()">${lbl}</button>`;
  let html='';
  if(others.length){
    html+=`<div class="wi-ctx-head">GROUPAGE</div>`;
  ...
  if(!row.saved){html+=`<div class="wi-ctx-sep"></div>`;html+=item('🗑 Remove row',`_wiDeleteRow(${rowId})`,true);}
...
function _wiSplit(rowId){
  ...
  rest.forEach(expId=>{
    ...
    WINTL.rows.push({id:++WINTL._seq,tripRecId:null,tripNo:'',exportIds:[expId],importId:null,
      ...
      truckPlate:'',trailerPlate:'',driverName:'',partnerName:'',
      loadingDate:exp?.fields['Loading DateTime']||'',
      carrierType:'owned',partnerRateExp:'',partnerRateImp:'',saved:false,dirty:false});
  });
...
function _wiViewExport(rowId){
  const row=WINTL.rows.find(r=>r.id===rowId);
  if(row?.exportIds?.[0]){navigate('orders_intl');setTimeout(()=>{if(typeof showIntlDetail==='function')showIntlDetail(row.exportIds[0]);},500);}
}
...
async function _wiSaveTrip(rowId,exportOnly=false){
  ...
  try{
    ...
    if(btn){btn.classList.remove('saving');btn.textContent=row.saved?'Update Trip':'🔗 Create Trip';}
