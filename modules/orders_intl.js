...
const _intlFilters = {};
let _clientsMap = {}; // recId → Company Name

// ── Helpers ───────────────────────────────────────
function _clientName(f) {
  // Try lookup fields first, then cache
  if (f['Client Name']) return f['Client Name'];
...
function _cleanSummary(s) {
  if (!s) return '—';
  // Formula returns e.g. "Ww Agrar Kg, AT"/ → strip quotes and trailing /
  return s.replace(/^["']|["']$/g, '').replace(/\/\s*$/, '').trim() || '—';
}

// ── Init ──────────────────────────────────────────
async function _loadClientsMap() {
  if (Object.keys(_clientsMap).length > 0) return; // already loaded
  try {
    const clients = await atGet(TABLES.CLIENTS);
    clients.forEach(r => { _clientsMap[r.id] = r.fields['Company Name'] || r.id.slice(-6); });
  } catch(e) { console.warn('Could not load clients map', e); }
}
    ...
    <div class="entity-layout">
      <div class="entity-list-panel">
        <div class="entity-toolbar" style="flex-wrap:wrap;gap:8px">
          <input class="search-input" style="max-width:210px" placeholder="🔍  Client / Location / Goods..." oninput="intlSearch(this.value)">
...
function _renderIntlTable(records) {
  ...
  const rows = records.slice(0,300).map(r => {
    ...
    const loading  = _cleanSummary(f['Loading Summary']);
    const delivery = _cleanSummary(f['Delivery Summary']);
    const orderNo  = f['Order Number'] || r.id.slice(-6);

    return `<tr onclick="selectIntlOrder('${r.id}')" id="irow_${r.id}" class="${sel}">
      <td style="white-space:nowrap">${hr}<strong style="color:var(--text);font-size:12px">${orderNo}</strong></td>
      <td>W${f['Week Number']||'—'}</td>
      <td>${dirB}</td>
      <td style="max-width:150px;overflow:hidden;text-overflow:ellipsis">${client}</td>
      <td style="max-width:140px;overflow:hidden;text-overflow:ellipsis">${loading}</td>
      <td style="max-width:140px;overflow:hidden;text-overflow:ellipsis">${delivery}</td>
      ...
      <td>${tripB}</td>
      <td>${f['Invoiced']?'<span class="badge badge-grey" title="Invoiced">INV</span>':''}</td>
    </tr>`;
  }).join('');
...
function selectIntlOrder(recId) {
  ...
  // Build loading stops
  let loadStops = '', delStops = '';
  for (let i=1;i<=5;i++) {
    const locArr = f[`Loading Location ${i}`];
    const locId  = Array.isArray(locArr) ? locArr[0] : null;
    const pal    = f[`Loading Pallets ${i}`];
    if (!locId) break;
    const name = locId.slice(-6); // fallback — locations not yet in cache
    loadStops += _dF(`Stop ${i}`, `${name}${pal?' — '+pal+' pal':''}`);
  }
  // Use summary fields for cleaner display
  const loadSum = _cleanSummary(f['Loading Summary']);
  const delSum  = _cleanSummary(f['Delivery Summary']);

  for (let i=1;i<=5;i++) {
    const locArr = f[`Unloading Location ${i}`];
    if (!Array.isArray(locArr) || !locArr[0]) break;
    const pal = f[`Unloading Pallets ${i}`];
    delStops += _dF(`Stop ${i}`, `${locArr[0].slice(-6)}${pal?' — '+pal+' pal':''}`);
  }

  panel.innerHTML = `
    <div class="detail-header">
      <div>
        <div class="detail-title" style="font-size:13px">${f['High Risk Flag']?'<span style="color:var(--danger);margin-right:4px">⚠</span>':''}${f['Order Number']||recId.slice(-6)}</div>
...
function _dF(l,v) {
  return `<div class="detail-field">
    ...
    <span class="detail-field-value">${v}</span>
  </div>`;
}
function _chk(l,v) {
  return `<div style="display:flex;align-items:center;gap:6px;font-size:12px;color:${v?'var(--success)':'var(--text-dim)'}">${v?'✅':'⬜'} ${l}</div>`;
}
  ...
  const body = `
    <div class="form-grid">
      <div class="form-field"><label class="form-label">Brand *</label>
        <select class="form-select" id="io_Brand"><option value="">— Select —</option>
          ${opt(['Petras Group','DPS Logistics'],'Brand')}</select></div>
      ...
      <div class="form-field"><label class="form-label">Pallet Type</label>
        <select class="form-select" id="io_Pallet_Type"><option value="">— Select —</option>
          ${opt(['EUR Pallet','Half Pallet','Industrial','Other'],'Pallet Type')}</select></div>
      <div class="form-field"><label class="form-label">Loading Pallets</label>
        <input class="form-input" type="number" id="io_Loading_Pallets_1"
          value="${f['Loading Pallets 1']||''}"></div>
      <div class="form-field"><label class="form-label">Price (€)</label>
        <input class="form-input" type="number" id="io_Price"
          value="${f['Price']||''}"></div>
      <div class="form-field"><label class="form-label">Reefer Mode</label>
        <select class="form-select" id="io_Refrigerator_Mode"><option value="">— Select —</option>
          ${opt(['Continuous','Start-Stop','Off'],'Refrigerator Mode')}</select></div>
      <div class="form-field"><label class="form-label">Gross Weight (kg)</label>
        <input class="form-input" type="number" id="io_Gross_Weight_kg"
          value="${f['Gross Weight kg']||''}"></div>
    ...
    <div style="display:flex;gap:20px;margin-top:14px;flex-wrap:wrap">
      <label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer">
        <input type="checkbox" id="io_Pallet_Exchange" ${f['Pallet Exchange']?'checked':''}> Pallet Exchange</label>
      <label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer">
        <input type="checkbox" id="io_High_Risk_Flag" ${f['High Risk Flag']?'checked':''}> High Risk</label>
      <label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer">
        <input type="checkbox" id="io_Docs_Ready" ${f['Docs Ready']?'checked':''}> Docs Ready</label>
      <label style="display:flex;align-items:center;gap:6px;font-size:13px;cursor:pointer">
        <input type="checkbox" id="io_Veroia_Switch" ${f['Veroia Switch ']?'checked':''}> Veroia Switch</label>
    </div>`;
...
async function saveIntlOrder(recId) {
  const fields = {};
  const strF = {io_Brand:'Brand',io_Direction:'Direction',io_Status:'Status',
    io_Carrier_Type:'Carrier Type',io_Goods:'Goods',io_Pallet_Type:'Pallet Type',
    io_Refrigerator_Mode:'Refrigerator Mode',io_Notes:'Notes'};
  ...
  const numF = {io_Week_Number:'Week Number',io_Loading_Pallets_1:'Loading Pallets 1',
    io_Price:'Price',io_Temperature:'Temperature °C',io_Gross_Weight_kg:'Gross Weight kg'};
  ...
  const chkF = {io_Pallet_Exchange:'Pallet Exchange',io_High_Risk_Flag:'High Risk Flag',
    io_Docs_Ready:'Docs Ready',io_Veroia_Switch:'Veroia Switch '};
