...
3. **Τι γράφτηκε ΟΝΤΩΣ** — δες την εγγραφή στη βάση/API, όχι το μήνυμα επιτυχίας.
4. **Τι γίνεται στην αποτυχία** — και είναι σωστό να ανοίγει ή να κλειδώνει;
5. **Έχει δικαίωμα** ο ρόλος ΚΑΙ ο `service_role` για αυτή την πράξη;

Γιατί υπάρχει ο κανόνας — και τα τέσσερα συνέβησαν στο ίδιο έργο (Αύγουστος 2026):
- Το `DELETE` κινήσεων παλετών γύριζε 403 από την πρώτη μέρα: ο `service_role`
  δεν είχε δικαίωμα διαγραφής. Το smoke είχε δοκιμάσει μόνο ότι οι οριστικές
  ΔΕΝ σβήνονται — ποτέ μια επιτυχή διαγραφή (ερώτηση 5).
- Η τιμολόγηση έδειχνε υπόλοιπο παλετών `0 (zero)` για κάθε πελάτη, γιατί
  διάβαζε άδειο πίνακα. Το άγραφο εμφανιζόταν ως ξοφλημένο (ερωτήσεις 2, 3).
- Gate που «κλειδώνει στην αποτυχία» θα σταματούσε ΟΛΗ την τιμολόγηση όσο
  έλειπε ένα view (ερώτηση 4).
- Λίστα πελατών κομμένη στους 500: ονόματα εμφανίζονταν ως `#1314` και οι μισοί
  πελάτες δεν επιλέγονταν καθόλου στη φόρμα (ερώτηση 2).

Ισχύει και αντίστροφα: αλλαγή σε πίνακα ή endpoint ⇒ βρες ΠΟΙΟ front end το
διαβάζει και τι θα δει.
...
Ό,τι αγγίζει φόρμες παραγγελιών ή εγγραφή σε ORDERS / NAT_ORDERS /
GROUPAGE_LINES / CONS_LOADS / NAT_LOADS **ανοίγει και δοκιμάζεται πριν το
push**. Στις 10/08 στάλθηκε ενοποίηση φόρμας χωρίς να ανοίξει η φόρμα ούτε μία
