...
3. For each chain node compute `expected_final` = `running_last` if present else `balance_sum`. If both are null ⇒ `needs_decision` («no ΠΡΟΟΔΕΥΤΙΚΟ and no ΥΠΟΛΟΙΠΟ column»). If the node has a ΠΡΟΟΔΕΥΤΙΚΟ column and `running_consistent` is false ⇒ `needs_decision` with `raw_final`, `opening_balance`, the breaks and `running_last` (do not "fix" it). If the node has no ΠΡΟΟΔΕΥΤΙΚΟ column and `raw_final ≠ balance_sum` ⇒ `needs_decision` likewise.
...
- A node with `running_consistent: false` inside a `ready` plan.
