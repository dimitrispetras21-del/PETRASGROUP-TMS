...
| 🟠 **Μερικώς** — live, ΔΕΝ έκλεισε | **1** | `PF-2`: ενώθηκε ο ΤΥΠΟΣ (και τα δύο καλούν `metrics.weeklyScore()`) αλλά ΟΧΙ οι ΕΙΣΟΔΟΙ. Live 5/8: κάρτα **44/100**, γράφημα **15/100**. Η κάρτα χρησιμοποιεί fallback **80** για άγνωστο on-time και `service_adherence`· το γράφημα την πραγματική τιμή εβδομάδας και `metrics.compliancePct`. Το χάσμα έπεσε από 11 σε 29 μονάδες αλλά **υπάρχει** |
...
| ⬜ **Ανοιχτά** | **137** | 15 επαληθευμένα μηχανικά (κάτω) · 2 νέα (`MA-7` `MA-8`) · 133 μη επαληθευμένα |
...
**Κλειστά και ζωντανά: 66 / 233** (28%). **+12 live** (Φάση 3) → **78 / 233** (33%). Το `PF-2` μένει ανοιχτό ως μερικώς.
