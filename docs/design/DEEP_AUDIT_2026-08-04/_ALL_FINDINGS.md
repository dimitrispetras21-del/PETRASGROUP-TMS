...
| 👤 **Μπλοκαρισμένα στον owner** | **10** | `MR-2` (γέφυρα εντολών→κόστους — απόφαση ροής) · `TL-1` `WS-1` `LO-1` `TR-1` (καταχώρηση) · `MS-1` `MT-1` `ML-1` (χρειάζονται και τη γέφυρα `MR-2`) · `ON-1` (διερεύνηση) · `AT-5` (απόφαση για τους demo λογαριασμούς) |
| ⚙️ **Μπλοκαρισμένα σε backend** | **5** | `PU-1` `PU-2` `PU-3` (PAT petras-assign) · `CO-2` (`TRIP_COSTS` → 404) · `AT-1` (το ίχνος σταμάτησε 28/7) |
| ⬜ **Ανοιχτά** | **76** | **70 επαληθευμένα** · **3 ανεπαλήθευτα**: `PN-4` `PN-5` (window.open/Safari — KNOWN_ISSUES M4, θέλουν δοκιμή σε iPad αποθήκης) `PP-4` (κόπωση επιβεβαιωτή — θέλει πραγματικό δελτίο) · **3 άλλο repo**: `PU-4` `PU-5` `PU-6` |
...
**ΤΡΕΧΟΝ ΑΘΡΟΙΣΜΑ (9/8/2026, μετά τη Φ8): 138 live (59%) · ❎ 4 · 👤 10 ·
⚙️ 5 · ⬜ 76 → 233** ✓
