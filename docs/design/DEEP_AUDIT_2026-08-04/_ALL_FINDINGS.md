...
| ⬜ **Ανοιχτά** | **133** | 11 επαληθευμένα μηχανικά (κάτω) · 122 μη επαληθευμένα — _τα «2 νέα» (`MA-7` `MA-8`) έκλεισαν στη Φάση 4_ |
...
**Άθροισμα 6/8/2026:**
✅ 22+14+14+16+13+3+1 = **83** · 🔵 2 · ❎ 1 · 👤 9 · ⚙️ 5 · ⬜ 133 →
**83+2+1+9+5+133 = 233** ✅
...
### 🤖 ΚΩΔΙΚΑΣ — μπορεί να γίνει χωρίς κανέναν άλλον

| Κωδικός | Σελίδα | Τι λείπει | Ώρες | Σοβ |
|---|---|---|---:|---|
| `PP-1` | weekly_pickups | 620 γραμμές υλοποιημένου κώδικα εκτός `NAV` — προσθήκη route/εγγραφής | 1 | **P0** |
| `SH-6` | shell | 3 σελίδες γράφουν δικό τους `topbarTitle` (8 αρχεία σήμερα) — αφαίρεση εγγραφών, ενιαίο breadcrumb | 1-2 | P1 |
| `CL-3` | clients | `entity.js:400` `toISOString().slice(0,7)` — ο «μήνας» γυρίζει σε UTC· λάθος μήνας τις πρώτες πρωινές ώρες της 1ης | 0.5 | P1 |
| `PR-1` `PR-3` | payroll | route χωρίς NAV και χωρίς `can()` gate — gate σαν του error_log (`router.js:335`) | 1 | P1+P2 |
| `D-5` | dashboard | «no matched pairs» αγγλικά ανάμεσα σε ελληνικά | 0.5 | P1 |
| `PA-2` | partners | Partner Rate ζει σε ORDERS/PARTNER_ASSIGN (ανά δρομολόγιο) — θέλει fetch + συγκέντρωση, όχι στήλη | 3-4 | P1 |
| `D-9` | dashboard | υπόλοιπο: `#0F172A` κύκλοι στα empty states (`ui.js:218,228`) → tokens | 0.5 | P2 |
| `CO-4` | costs | `label: 'Costs (soon)'` — **ΠΡΟΣΟΧΗ**: πιθανώς λυμένο στον κλάδο `design/ux-batch-1` («hide unbuilt nav items»)· έλεγξε πριν το ξαναγράψεις | 0.5 | P2 |
| `PA-5` | partners | πεδίο `'Adress'` (ένα «d») — rename θέλει συντονισμό με backend/βάση | 1 | P2 |
| `LG-7` | login | καμία `<form>`/`<label>`, κουμπί `onclick` αντί submit | 1 | P2 |
| εκτός αρίθμησης | όλο το app | z-index 17→6 σημασιολογική κλίμακα · font-size 30→7 · hex σε modules (ternaries/SVG attrs, όπου το `var()` δεν δουλεύει) | 6-10 | P2 |

Συν: τα **122 μη επαληθευμένα ανοιχτά** θέλουν πέρασμα στο παραγωγικό πριν
από οποιονδήποτε προγραμματισμό — ο πραγματικός αριθμός μπορεί να είναι
μικρότερος.
...
### Τι ΔΕΝ επαληθεύτηκε (6/8) και γιατί

- Ο τοπικός server **δεν περνά CORS στον Worker (403)** — ό,τι θέλει δεδομένα
  ελέγχεται ΜΟΝΟ στο παραγωγικό. Όλοι οι σημερινοί έλεγχοι έγιναν εκεί.
- Τα **122 μη επαληθευμένα ανοιχτά**: απόδειξη «live text»/screenshot — δεν
  ξαναελέγχθηκαν. Το «133 ανοιχτά» είναι άνω όριο.
- `MR-3` (το 3.913px δεν ξαναμετρήθηκε) · `WI-8` (η επιστροφή στην τρέχουσα
  εβδομάδα δεν δοκιμάστηκε) → παραμένουν 🔵.
- Οπτική επαλήθευση των νέων κατωφλιών στα 1024-1100px: το resize του browser
  εργαλείου δεν έπιασε (viewport κλειδωμένο στα 1600px). Τα 3 auto-fit grids
  μετρήθηκαν σε πραγματικά δοχεία 9 πλατών (έγκυρο — δεν εξαρτώνται από media
  queries)· οι κανόνες 1100→1024 επαληθεύτηκαν μόνο ως κανόνες στο CSSOM.
