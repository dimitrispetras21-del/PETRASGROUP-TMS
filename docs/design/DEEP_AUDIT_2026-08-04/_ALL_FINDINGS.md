...
| 👤 **Μπλοκαρισμένα στον owner** | **13** | `PN-4` `PN-5` (θέλουν δοκιμή εκτύπωσης στο iPad της αποθήκης — δεν υπάρχει πρόσβαση από εδώ) · `PP-4` (θέλει πραγματικό δελτίο παλετών σε AI extraction) · `MR-2` (γέφυρα εντολών→κόστους — απόφαση ροής) · `TL-1` `WS-1` `LO-1` `TR-1` (καταχώρηση) · `MS-1` `MT-1` `ML-1` (χρειάζονται και τη γέφυρα `MR-2`) · `ON-1` (διερεύνηση) · `AT-5` (απόφαση για τους demo λογαριασμούς) |
| ⚙️ **Μπλοκαρισμένα σε backend** | **5** | `PU-1` `PU-2` `PU-3` (PAT petras-assign) · `CO-2` (`TRIP_COSTS` → 404) · `AT-1` (το ίχνος σταμάτησε 28/7) |
| ⬜ **Ανοιχτά** | **43** | **40 επαληθευμένα** · **3 άλλο repo**: `PU-4` `PU-5` `PU-6` |
...
**ΤΡΕΧΟΝ ΑΘΡΟΙΣΜΑ (10/8/2026 βράδυ, μετά τη Φ10 ΠΛΗΡΗ + PU-8): 168 live
(72%) · ❎ 4 · 👤 13 · ⚙️ 5 · ⬜ 43 (40 επαληθευμένα + 3 άλλο repo) → 233** ✓
