...
✅ ΥΛΟΠΟΙΗΘΗΚΑΝ (ίδιο branch feat/weekly-v3-ui, commit Φάσης Β):
- ✨ προτάσεις με ✓ — `_wk3Busy/_wk3Suggest/_wk3Accept`: πρώτα δικός
  στόλος (οδηγός διαθέσιμος κατά Χ+2 για την ημ. φόρτωσης + φορτηγό
  ελεύθερο), αποδοχή μέσω του ΚΑΝΟΝΙΚΟΥ `_wiSaveFromPopover` (validations,
  T1 confirm, locks, PA/VS sync — μηδέν νέο write-path). ✨N στο tally.
- Πάνελ «Οδηγοί · 561/2006 (Χ+2)» — ΟΛΟΙ οι active drivers, 3 καταστάσεις
  (διαθέσιμος dashed πράσινο πρώτος / δρομ.→ημέρα·τόπος / ανάπ.→Χ+2),
  tooltip με ⚡Χ+1 κατ' εξαίρεση.

⛔ ΕΚΚΡΕΜΟΥΝ (θέλουν απάντηση owner — μία ερώτηση τη φορά):
- δεξί κλικ import → μεταφορά εβδομάδας (shift ημερομηνιών ±7 ή άλλο;)
- κατάσταση ΜΑΤΑΙΩΘΗΚΕ (ποιο πεδίο/τιμή Status στα ORDERS;)
