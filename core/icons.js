// ═══════════════════════════════════════════════════════════
// CORE — ICON SYSTEM (Lucide-style outline SVGs)
// Replaces emoji icons throughout the app with consistent SVG.
// Usage: icon('warning', 16) → '<svg...>'
// ═══════════════════════════════════════════════════════════
...
const ICONS_LIB = {
  ...
  user_check: '<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><polyline points="17 11 19 13 23 9"/>',
  // Vehicles / transport
  ...
  // Party/celebration
  party: '<path d="M5.8 11.3L2 22l10.7-3.79M4 3h.01M22 8h.01M15 2h.01M22 20h.01M22 2L17 7l-1.5-1.5M22 13l-4 4-1.5-1.5M8 5L3 10l1.5 1.5M13 18l-4-4 1.5-1.5"/>',
  ...
  target_v2: '<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>',
  // Sidebar additions
  layout_grid: '<rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/>',
  globe: '<circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>',
  home: '<path d="M3 9.5L12 2l9 7.5V20a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V9.5z"/><path d="M9 22V12h6v10"/>',
 ...
 * @param {string} name - icon key from ICONS_LIB
 * @param {number} [size=16] - width/height in px
 * @param {string} [extraCls=''] - extra CSS classes
 * @param {number} [stroke=1.75] - stroke width
 */
function icon(name, size = 16, extraCls = '', stroke = 1.75) {
  const paths = ICONS_LIB[name];
  if (!paths) return '';
  return `<svg class="icon ${extraCls}" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${stroke}" stroke-linecap="round" stroke-linejoin="round">${paths}</svg>`;
}
